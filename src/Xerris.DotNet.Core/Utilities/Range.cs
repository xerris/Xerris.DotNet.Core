...
public class Range<T> : IEquatable<Range<T>> where T : IComparable<T>
{
    ...
    [IgnoreForReflectionEquals(
        "Imagine iterating over every day from 2014-08-01 to 9999-08-01 with Reflection Equals. That will take a while just to do a ReflectionEquals.")]
    ...
    {
        if (incAction == null) throw new NullReferenceException("An incrementor action is required.");
        for (var current = Start; Includes(current); current = incAction(current)) action(current);
    }

    public virtual void BackwardForEach(Func<T, T> decrementorAction, Action<T> action)
    {
        if (decrementorAction == null) throw new NullReferenceException("An decrementor action is required.");
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            return ((incrementor != null ? incrementor.GetHashCode() : 0) * 397) ^
                   (tuple != null ? tuple.GetHashCode() : 0);
        }
    }
