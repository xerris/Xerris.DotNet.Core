...
public static class ValidationExtensions
{
    ...
    private static readonly Regex NumericRegex = new(@"^([-+]?[0-9]\d*(\.\d+)?|[0]*\.\d\d*)$");
    ...
    private static readonly Regex PostalCodeRegex =
        new(@"^[ABCEGHJKLMNPRSTVXY]\d[ABCEGHJKLMNPRSTVWXYZ][\s]{1}\d[ABCEGHJKLMNPRSTVWXYZ]\d$");
    ...
    public static Validation IsFalse(this Validation validation, bool? val, string message)
    {
        return val is false ? validation : validation.AddException(new ValidationException(message));
    }
    ...
    {
        return string.IsNullOrEmpty(theValue?.ToString()) || !NumericRegex.IsMatch(theValue.ToString() ?? string.Empty)
    ...
    {
        return criteria.Length != 0 ? validation.Add(new ValidationException(message)) : validation;
    }
